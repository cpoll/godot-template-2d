...
var joined_devices = [null, null, null, null]
var countdown = null
...
func handle_input():
    ...
    var unjoined_devices = devices.filter(func(d): return d not in joined_devices)
    ...
    for device in unjoined_devices:
        if MultiplayerInput.is_action_just_pressed(device, "Confirm"):
            ...
            var first_empty_player_slot = joined_devices.find(null)
            if first_empty_player_slot > -1:
                joined_devices[first_empty_player_slot] = device
    
    # Handle joined device input
    for i in joined_devices.size():
        var device = joined_devices[i]
        if device == null:
            continue
            
        if MultiplayerInput.is_action_just_pressed(device, "Back"):
            joined_devices[i] = null
            stop_countdown()
...
func update_ui():
    for i in joined_devices.size():
        if joined_devices[i] == null:
            p_labels[i].text = "Press Confirm to Join"
        else:
            p_labels[i].text = "Joined!"
